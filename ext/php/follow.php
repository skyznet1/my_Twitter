...
$getFollow = $dbh ->prepare("SELECT * FROM user_follow INNER JOIN users u WHERE id_follower = :id_user AND u.id = user_follow.id_followed LIMIT $offset, $no_of_records_per_page");
$getFollow -> execute(array(
    'id_user' => $_SESSION['id']
));
$follow = $getFollow ->fetchAll();
